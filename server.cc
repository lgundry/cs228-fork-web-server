...
#include <sys/stat.h> // file streams
#include <fcntl.h> // file streams
#include <sys/file.h> // file streams
	...
	while (true)
	{
		...
		if (pid == 0)
		{
			...
			try
			{
				...
				// open log file
				int logfd = open((LOG_FILES + "access.log").c_str(), O_WRONLY | O_APPEND | O_CREAT);
				if (logfd == -1)
				...
				if (fileRequested == "404.html")
				{
					...
					log = gethostbyaddr(&sa.sin_addr.s_addr, sizeof(sa.sin_addr), AF_INET)->h_name + string(" - - [")
						+ string("25/Mar/2024:23:27:41 -0500") + string("] \"GET /") + fileRequested + string(" HTTP/1.1\" 404 ") + to_string(fileSize) + string(" \"") + referrer + string("\"\n");
				...
				else
				{
					...
					log = gethostbyaddr(&sa.sin_addr.s_addr, sizeof(sa.sin_addr), AF_INET)->h_name + string(" - - [")
						+ string("25/Mar/2024:23:27:41 -0500") + string("] \"GET /") + fileRequested + string(" HTTP/1.1\" 200 ") + to_string(fileSize) + string(" \"") + referrer + string("\"\n");
					write(logfd, log.c_str(), log.size());
				}
